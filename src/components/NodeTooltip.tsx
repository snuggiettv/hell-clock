 ...
 */
const NodeTooltip: React.FC<{ data: TipData | null; containerSelector?: string }> = ({
  data,
  containerSelector,
}) => {
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
        {data.iconUrl && (
          <img
            ...
            width={24}
            height={24}
            style={{ flex: '0 0 auto', opacity: 0.95 }}
          />
        )}
        <div style={{ fontWeight: 700, fontSize: 15, letterSpacing: 0.2, flex: 1 }}>{data.title}</div>
        {data.state && (
          <div
            style={{
              fontSize: 11,
