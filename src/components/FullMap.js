...
import MapNodeTooltip from './MapNodeTooltip';
...
import { aggregateTotals } from '../utils/aggregateTotals';
...
// ---- Tooltip presets ----
const reqSatisfied = (need, have) => !!need && ['Red', 'Green', 'Blue'].every(k => (have?.[k] ?? 0) >= (need[k] ?? 0));
...
const isMarysTitle = (t) => /three\s*marys?|tres\s*marias/i.test(t ?? '');
...
/* ---------- Storage keys ---------- */
const COLOR_KEY = (id) => `constellation:${id}:color`;
const COMPLETE_KEY = (id) => `constellation:${id}:complete`;
const ALIGN_KEY = (id) => `constellation:${id}:align`;
const ACTIVE_NODES_KEY = (slug) => `constellation:${slug}:activeNodes`; // Marys per-node
/* ---------- Data URLs ---------- */
...
function materializeRowWithOverrides(row, overrides) {
    const statKey = row.statKey ? String(row.statKey).trim() : '';
    ...
    const scaled = (vKey, perKey) => effectiveValue({
        rank,
        value: num(row[vKey]),
        valuePerLevel: num(row[perKey]),
    });
    // Build payload with multiple value slots so longer patterns fill correctly
    const payload = { pattern, statKey };
    if (Array.isArray(row.rawNumbers)) {
        payload.rawNumbers = row.rawNumbers;
    }
    // value / valuePerLevel
    if ('value' in row || 'valuePerLevel' in row) {
...
function useAffixOverrides() {
    const [affixOverrides, setAffixOverrides] = React.useState({});
    React.useEffect(() => {
        let alive = true;
        (async () => {
            try {
...
const englishName = (def) => {
    const arr = (def?.nameLocalizationKey ?? def?.nameKey);
...
const loadColor = (id, fallback = '#ffffff') => {
    try {
...
const isComplete = (slug) => {
    try {
        return localStorage.getItem(COMPLETE_KEY(slug)) === '1';
    }
    catch {
        return false;
    }
};
const meets = (h, n) => !n || (h.Red >= (n.Red || 0) && h.Green >= (n.Green || 0) && h.Blue >= (n.Blue || 0));
const missingText = (h, n) => {
    if (!n)
        return '';
    const miss = [];
    const d = (x, y) => Math.max(0, (y ?? 0) - x);
...
const loadAlign = (id) => {
    try {
        ...
        return { x: +j?.x || 0, y: +j?.y || 0, scale: +j?.s || +j?.scale || 1 };
    }
    catch {
        return { x: 0, y: 0, scale: 1 };
    }
};
const pickSize = (pg) => pg?.size?.width && pg?.size?.height
...
const placementFor = (e, map) => map[e.id] || map[String(e.group?.definition?.id ?? '')] || { x: 0, y: 0, scale: 1, rotation: 0 };
/* ---------- Affixes / tooltip helpers ---------- */
// Build a map { nodeId -> rows[] }
function buildAffixIndex(rows) {
    const m = new Map();
    for (const r of rows) {
...
function keyCandidates(g, n) {
    ...
    const nameish = [];
    if (label)
        nameish.push(label, label.toLowerCase());
    if (nodeId)
        nameish.push(nodeId, nodeId.toLowerCase());
    const keys = [];
    for (const nm of nameish)
        if (defId)
            keys.push(`${defId}:${nm}`);
    for (const nm of nameish)
        if (slug)
...
function lookupAffixes(idx, g, n) {
    const byId = idx.get(String(n?.id ?? ''));
    if (byId && byId.length)
        return { key: String(n.id), rows: byId };
...
function isMaryConstellation(e) {
...
function extractIconKeyFromRow(r) {
    for (const k of Object.keys(r)) {
        if (/icon|sprite/i.test(k) && typeof r[k] === 'string' && r[k])
            return String(r[k]);
    }
    return undefined;
}
function iconKeyFromAffixes(affixes) {
...
function iconUrlForUrl(affixes) {
    const key = iconKeyFromAffixes(affixes);
    return key ? `${ICON_BASE}${key}.png` : undefined;
}
/* ---------- Viewport math ---------- */
function clampCentered(v, vw, vh, worldW, worldH, minK, maxK, padX) {
...
function animateTo(set, get, el, to, worldW, worldH, minK, maxK, padX, dur = 1500) {
    const start = performance.now(), from = get();
    let raf = 0;
    const tick = (now) => {
...
function deriveAvailable(nodes, adj, active, roots) {
    const out = new Set();
    if (active.size === 0) {
    ...
    for (const n of nodes) {
        ...
        if (anyNeighborActive)
            out.add(n.id);
    }
    return out;
}
function cascadeDeactivate(startId, adj, active, roots) {
    active.delete(startId);
    const keep = new Set();
    const q = [...roots].filter(r => active.has(r));
    if (q.length === 0) {
        active.clear();
        return;
    }
...
/* ---------- Art visibility ---------- */
const isWIP = (id, label) => /^wip[_-]/i.test(id) || /^wip[_-]/i.test((label ?? ''));
function reqFor(entry, reqs) {
...
export default function FullMap() {
    ...
    const [devotion, setDevotion] = React.useState({ Red: 0, Green: 0, Blue: 0 });
    const [tip, setTip] = React.useState(null);
    const [summaryConstId, setSummaryConstId] = React.useState(null);
    const [hovered, setHovered] = React.useState(null);
    // preview/test active nodes per constellation
    const [testActive, setTestActive] = React.useState(() => new Map());
    /* ---------- Refs ---------- */
    ...
    const maryActiveRef = React.useRef(new Set());
    const loadMaryActive = React.useCallback((slug) => {
        try {
            const raw = localStorage.getItem(ACTIVE_NODES_KEY(slug));
            if (raw)
                maryActiveRef.current = new Set(JSON.parse(raw));
        }
        catch { }
    }, []);
    const saveMaryActive = React.useCallback((slug, ids) => {
        try {
            localStorage.setItem(ACTIVE_NODES_KEY(slug), JSON.stringify([...ids]));
        }
        catch { }
    }, []);
    /* ---------- Load data ---------- */
    React.useEffect(() => {
        ...
        (async () => {
            try {
                const r = await fetch(getMasterUrl(), { cache: 'no-store' });
                const j = await r.json();
                const list = Array.isArray(j?.constellationsDetails) ? j.constellationsDetails : [];
                const seen = new Map();
                const out = [];
                for (const g of list) {
                    if (!g?.definition)
                    ...
                    const pg = parseGroupToGraph({ id, artBase: `${import.meta.env.BASE_URL}constellations/`, group: g });
                    ...
                    out.push({
                        ...
                        edges: Array.isArray(pg?.edges)
                            ? pg.edges.map((e) => ({ source: String(e.source), target: String(e.target) }))
                            : [],
                    });
                }
                if (ok)
                ...
                console.error('[FullMap] master', e);
                if (ok)
                    setEntries([]);
            }
        })();
    ...
    React.useEffect(() => {
        ...
        (async () => {
            try {
                const r = await fetch(getPlacementsUrl(), { cache: 'no-store' });
                const j = await r.json();
                if (!ok)
                    return;
                setPlacements(j?.map || {});
                setColorMap(j?.colors || {});
            }
            catch {
                if (ok) {
                    setPlacements({});
    ...
    React.useEffect(() => {
        ...
        (async () => {
            try {
                const r = await fetch(getRequirementsUrl(), { cache: 'no-store' });
                const j = await r.json();
                if (ok)
                    setReqs(j || {});
            }
    ...
    React.useEffect(() => {
        if (!testModeEnabled) {
            try {
                const mary = entries.find((e) => isMaryConstellation(e));
                if (mary)
                    localStorage.removeItem(ACTIVE_NODES_KEY(mary.id));
            }
            catch { }
        }
    ...
    const activeNodeIds = React.useMemo(() => {
        ...
        for (const e of entries) {
            if (testModeEnabled) {
                for (const id of (testActive.get(e.id) ?? []))
                    ids.add(id);
            }
            else {
                if (isComplete(e.id))
        ...
        for (const id of activeNodeIds) {
            ...
            for (const r of list) {
                rows.push({
                    ...
                    valuePerLevel: Number(r.valuePerLevel ?? 0),
    ...
    const { lines: globalLines } = React.useMemo(() => aggregateTotals(globalRows, affixOverrides), [globalRows, affixOverrides]);
    /* ---------- World bounds ---------- */
    const world = React.useMemo(() => {
        if (!entries.length)
            return { width: 0, height: 0, ox: 0, oy: 0 };
    ...
    React.useLayoutEffect(() => {
        ...
        if (!el || !entries.length || !world.width)
            return;
        const vw = el.clientWidth, vh = el.clientHeight;
        ...
        if (focusBox) {
            ...
            const t = window.setTimeout(() => {
                ...
                el._cancelZoom = cancel;
                window.setTimeout(() => { animatingRef.current = false; }, FOCUS_DUR + 40);
            }, FOCUS_DELAY);
            return () => { clearTimeout(t); el._cancelZoom?.(); };
        }
    ...
    React.useEffect(() => {
        ...
        const onWheel = (e) => {
            ...
            const clamped = clampCentered({ k: nextK, x: nx, y: ny }, host.clientWidth, host.clientHeight, world.width, world.height, minKRef.current, MAX_K, PAD_X);
            setView(clamped);
        };
    ...
    const topology = React.useMemo(() => {
        const m = new Map();
        for (const e of entries)
            m.set(e.id, { adj: buildAdj(e.nodes, e.edges), roots: inferRoots(e.nodes, e.edges) });
        return m;
    }, [entries]);
    const availableBySlug = React.useMemo(() => {
        const m = new Map();
        for (const e of entries) {
            const topo = topology.get(e.id);
            if (!topo)
                continue;
            const active = testActive.get(e.id) ?? new Set();
            m.set(e.id, deriveAvailable(e.nodes, topo.adj, active, topo.roots));
        }
        return m;
    }, [entries, topology, testActive]);
    const setActiveFor = React.useCallback((slug, updater) => {
        setTestActive(prev => {
    ...
    const isNodeActiveForSummary = React.useCallback((e, nodeId) => {
        if (testModeEnabled) {
    ...
    const summary = React.useMemo(() => {
        ...
        if (testModeEnabled) {
            const set = testActive.get(e.id) ?? new Set();
            if (set.size) {
                for (const id of set) {
    ...
    const recomputeTotals = React.useCallback(() => {
        ...
        if (mary) {
            ...
            const shouldUsePerNode = testModeEnabled ? true : !isCompleteNow(mary);
            if (shouldUsePerNode) {
                const activeIds = testModeEnabled ? (testActive.get(mary.id) ?? new Set()) : new Set();
                // Map to original definition nodes to read affix text
                const defNodes = mary.group?.definition?.nodes || [];
                const defByKey = new Map(defNodes.map((dn) => [String(dn?.id ?? dn?.name ?? ''), dn]));
                for (const id of activeIds) {
                    ...
                    for (const a of affs) {
                        ...
                        if (/\bfury\s+devotion\b/.test(text)) {
                            totals.Red += 1;
                            break;
                        }
                        if (/\bdiscipline\s+devotion\b/.test(text)) {
                            totals.Green += 1;
                            break;
                        }
                        if (/\bfaith\s+devotion\b/.test(text)) {
                            totals.Blue += 1;
                            break;
                        }
                    }
        ...
        try {
            ...
            window.dispatchEvent(new CustomEvent('devotion-totals-changed', { detail: totals }));
        }
        catch { }
    ...
    React.useEffect(() => {
        const handler = (ev) => {
            ...
            if (cid !== mary.id && cid !== String(mary.group?.definition?.id ?? ''))
                return;
        ...
        window.addEventListener('node-activation-changed', handler);
        return () => window.removeEventListener('node-activation-changed', handler);
    }, [entries, saveMaryActive, recomputeTotals]);
    /* ---------- Render viewport ---------- */
    const viewport = (_jsxs("div", { ref: wrapRef, onMouseDown: onMouseDown, onMouseMove: onMouseMove, onMouseUp: stopDrag, onMouseLeave: stopDrag, style: {
            position: 'relative',
            width: '100%',
            height: '100%',
            overflow: 'hidden',
            ...
            touchAction: 'none',
        }, children: [_jsx(DevotionOverlay, { totals: devotion, containerRef: contentRef, inset: LETTERBOX.HEADER_H + LETTERBOX.SIDE_PAD }), _jsx("div", { style: {
                    position: 'absolute',
                    left: 0,
                    top: 0,
                    width: world.width,
                    ...
                    const transform = `translate(${p.x + world.ox}px, ${p.y + world.oy}px) ${p.rotation ? `rotate(${p.rotation}deg)` : ''} scale(${p.scale})`;
                    const toLocal = (pt) => ({
                    ...
                    const active = testActive.get(g.id) ?? new Set();
                    ...
                    const complete = testModeEnabled
                        ...
                        : isComplete(g.id);
                    // gating for art
                    const has99Gate = !!need && ['Red', 'Green', 'Blue'].some((c) => Number(need[c]) === 99);
                    ...
                    const handleRight = (e) => {
                        if (!testModeEnabled || (!ok && !isMaryConstellation(g)))
                            return;
                        e.preventDefault();
                    ...
                    const isActive = (id) => complete ? true : testModeEnabled ? active.has(id) : ok;
                    return (_jsxs("div", { style: {
                            ...
                            filter: ok ? 'none' : 'grayscale(0.2)',
                        }, title: ok ? g.label : `${g.label}\nLocked — ${missingText(devotion, need)}`, children: [_jsxs("div", { style: { position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 0 }, children: [showBlur && (_jsx("div", { style: {
                                            position: 'absolute',
                                            inset: 0,
                                        ...
                                        } }))] }), _jsx("svg", { width: CW, height: CH, style: {
                                    position: 'absolute',
                                    left: 0,
                                    top: 0,
                                    pointerEvents: 'none',
                                    ...
                                    const bright = complete
                                        ...
                                        : testModeEnabled
                                            ...
                                            : ok;
                                    return (_jsx("line", { x1: a.x, y1: a.y, x2: b.x, y2: b.y, stroke: bright ? color : 'rgba(185,205,225,0.45)', strokeWidth: 2, opacity: bright ? 0.9 : 0.6, vectorEffect: "non-scaling-stroke" }, i));
                                }) }), _jsx("div", { style: {
                                    position: 'absolute',
                                    inset: 0,
                                    zIndex: 2,
                                    ...
                                    const stateClass = isActive(n.id)
                                        ...
                                        : isAvail
                                            ? 'node-unlocked'
                                            : 'node-locked';
                                    return (_jsx("div", { "data-id": n.id, className: `diamond-node ${stateClass}`, onMouseDown: (e) => { e.stopPropagation(); }, onClick: (e) => {
                                            e.stopPropagation();
                                            ...
                                            setTestActive((prev) => {
                                                ...
                                                if (wasActive) {
                                                    ...
                                                    if (!isAvail)
                                                        return prev; // only allow if available/root
                                                    set.add(n.id);
                                                }
                                                next.set(g.id, set);
                                                // --- Marys bookkeeping ---
                                                if (isMaryConstellation(g)) {
                                                    ...
                                                    if (wasActive)
                                                        maryNext.delete(n.id);
                                                    else
                                                        maryNext.add(n.id);
                                                    maryActiveRef.current = maryNext;
                                                    try {
                                                        saveMaryActive(g.id, maryNext);
                                                    }
                                                    catch { }
                                                }
                                                try {
                                                    window.dispatchEvent(new CustomEvent('node-activation-changed', {
                                        ...
                                        }, onContextMenu: testModeEnabled ? handleRight : undefined, onMouseEnter: (e) => {
                                            ...
                                            if (isMaryConstellation(g)) {
                                                const defNodes = g.group?.definition?.nodes || [];
                                                const map = new Map(defNodes.map((dn) => [String(dn?.id ?? dn?.name ?? ''), dn]));
                                                ...
                                                if (dn) {
                                                    ...
                                                    if (hasDisc)
                                                        effectLines.push('+1 Discipline');
                                                    if (hasFaith)
                                                        effectLines.push('+1 Faith');
                                                }
                                            }
                                            ...
                                            for (const dn of defNodes) {
                                                ...
                                                const enArr = (dn?.nameLocalizationKey ?? dn?.nameKey);
                                                const enTxt = Array.isArray(enArr) ? enArr.find(k => k?.langCode === 'en')?.langTranslation : undefined;
                                                const enKey = enTxt ? String(enTxt).trim() : '';
                                                if (idKey)
                                                    defByKey.set(idKey, dn);
                                                if (nameKey)
                                                    defByKey.set(nameKey, dn);
                                                if (enKey) {
                                                    defByKey.set(enKey, dn);
                                                    defByKey.set(slugify(enKey), dn);
                                                }
                                            }
                                            ...
                                            if (defNode) {
                                                rawLines = (defNode.affixes || [])
                                                    .map((ax) => {
                                                    const en = (ax.description || []).find((loc) => loc.langCode === 'en');
                                            ...
                                            const reqTotals = need
                                                ? {
                                                    Red: Number(need.Red || 0),
                                                    Green: Number(need.Green || 0),
                                                    Blue: Number(need.Blue || 0),
                                                }
                                                : undefined;
                                            const bonusTotals = g.group?.definition?.masteredDevotionGranted || undefined;
                                            setTip({
                                                show: true,
                                                x: e.clientX,
                                                y: e.clientY,
                                                ...
                                                effectLines,
                                                iconUrl,
                                                countText: ((active.size > 0 || complete) && (ok || testModeEnabled))
                                        ...
                                        }, onMouseLeave: () => { setTip(null); setHovered(null); }, style: {
                                            position: 'absolute',
                                            left: pt.x - size / 2,
                                            top: pt.y - size / 2,
                                            cursor: testModeEnabled ? 'pointer' : 'default',
                                            ['--cn-size']: `${size}px`,
                                            ['--cn-tint']: color, // constellation tint
                                            ['--cn-glow']: color, // keep if used elsewhere
                                        } }, n.id));
                                }) })] }, g.id));
                }) })] }));
    /* ---------- Render ---------- */
    if (!isPreview) {
        return (_jsxs("div", { style: { position: 'relative', height: '100%', minHeight: 0 }, children: [_jsxs("div", { id: "letterbox-content", ref: contentRef, style: {
                        position: 'absolute',
                        inset: 0,
                        overflow: 'hidden',
                        ...
                        zIndex: 1,
                    }, children: [_jsx("div", { id: "viewport-clip", style: { position: 'absolute', top: 0, left: 0, bottom: 0, right: RIGHT_SAFE, overflow: 'hidden', zIndex: 1 }, children: viewport }), _jsx("div", { "aria-hidden": true, style: {
                                position: 'absolute',
                                top: 0,
                                right: 0,
                                bottom: 0,
                                ...
                                zIndex: 2,
                                pointerEvents: 'none',
                            } }), globalSummary && (_jsx(MapStatSummary, { title: globalSummary.title, rows: globalSummary.rows }))] }), _jsx("div", { style: {
                        position: 'absolute',
                        top: 0,
                        left: 0,
                        right: 0,
    ...
    return (_jsxs("div", { id: "letterbox-root", style: { position: 'fixed', inset: 0, overflow: 'hidden', background: '#060a12' }, children: [_jsx("div", { "aria-hidden": true, style: {
                    ...
                    pointerEvents: 'none',
                } }), _jsxs("div", { id: "letterbox-content", ref: contentRef, style: {
                    position: 'absolute',
                    inset: 0,
                    overflow: 'hidden',
                    zIndex: 1,
                    background: 'rgba(5,8,14,0.55)',
                    backdropFilter: 'blur(2px)',
                }, children: [viewport, globalSummary && (_jsx(MapStatSummary, { title: globalSummary.title, rows: globalSummary.rows }))] }), _jsx("div", { style: {
                    position: 'absolute',
                    top: 0,
                    left: 0,
                    right: 0,
