 ...
 */
function parseNumbersInOrder(s: string): number[] {
 ...
 */
function getEnDescription(desc?: any): string | null {
  if (!desc) return null;
...
// --- Types from the remote JSON -------------------------------------------

/**
 * ConstellationsConfig
 *  Minimal typed view of the JSON we read. It's purposefully permissive so
 *  upstream schema tweaks won't break extraction.
 */
  ...
  constellationsDetails: Array<{
    definition: {
      ...
      nodes: Array<{
        name?: string;
        id?: string;
        sprite?: string; // icon key (e.g., "Icon_Physical_Damage")
        affixes?: Array<{
          type?: string;                     // raw affix type label from export
          eStatDefinition?: string;          // e.g., "PhysicalDamage"
          eCharacterIncrement?: string;      // behavior/increment markers
          eDevotionCategory?: string;        // devotion category if present
          statModifierType?: string;         // "Additive" | "Multiplicative..." | etc
          value?: number;                    // structured value (prefer this)
...
// This is the *flat* row we emit for each affix. It is the source-of-truth
// that feeds your tooltip builder and the MapStatSummary.
//
// NOTE: We add `type?: string` so `rows.push({ type: aff.type, ... })` is valid.
/**
 * ConstellationAffixRow
 *  The flat shape the rest of the UI consumes. Keep this backward compatible.
 */
export type ConstellationAffixRow = {
  nodeId: string;              // "<constellationId>:<node name/id>"
  icon: string;                // raw sprite key (UI layer builds the URL)
  pattern: string;             // EN text with numbers normalized to '#'
  value: number | null | undefined; // structured value (prefer for math)
  statKey: string;             // best-effort key used for bucketing/labels
  statModifierType: string;    // "Additive", "MultiplicativeAdditive", "Unknown", ...
  rawNumbers: number[];        // digits parsed from EN (fallback/reference only)
  type?: string;               // (optional) original affix.type from data export
};
 ...
 */
export function extractConstellationAffixes(
  raw: ConstellationsConfig
): ConstellationAffixRow[] {
  ...
  for (const c of raw.constellationsDetails ?? []) {
    ...
    for (const node of def.nodes) {
      ...
      const nodeId =
        constId !== undefined ? `${constId}:${nodeName}` : String(nodeName);
      ...
      for (const aff of node?.affixes ?? []) {
        ...
        if (!enText) continue; // skip non-EN / empty lines
