...
type RFNodeLike = {
  ...
  data?: {
    ...
    affixes?: Array<{
      ...
      eDevotionCategory?: string; // Red | Green | Blue
      statModifierType?: string;  // "Additive", "MultiplicativeAdditive", etc.
      value?: number;
...
export type TotalsResult = {
  ...
  devotion: Record<'Red' | 'Green' | 'Blue', number>;
  details: {
    contributingNodes: Record<string, { value: number; rows: any[] }>;
  };
};

/** Treat multiplicatives as factors → convert to delta (1.2 → 0.2). Everything else is raw. */
function normalizeValue(v: number, statModifierType?: string) {
  const mt = (statModifierType || '').toLowerCase();
  const isMult = mt.includes('multiplicative');
  if (!Number.isFinite(v)) return 0;
  if (isMult && v > 1) return v - 1; // factor -> delta
  return v;
}

/** If key is a devotion color, return the bucket name; otherwise null. */
function devotionBucket(statKey?: string): 'Red' | 'Green' | 'Blue' | null {
  if (!statKey) return null;
  if (/^Devotion_(Red|Green|Blue)$/i.test(statKey)) {
    return statKey.split('_')[1] as 'Red' | 'Green' | 'Blue';
  }
  if (/^(Red|Green|Blue)$/i.test(statKey)) {
    return statKey as 'Red' | 'Green' | 'Blue';
  }
  return null;
}
 ...
 */
export function aggregateFromNodes(nodes: RFNodeLike[]): TotalsResult {
  const byStat: Record<string, number> = Object.create(null);
  const devotion: Record<'Red' | 'Green' | 'Blue', number> = { Red: 0, Green: 0, Blue: 0 };
  const contributingNodes: TotalsResult['details']['contributingNodes'] = {};

  for (const n of nodes) {
    if (!n?.data?.isActivated) continue;
    ...
    for (const a of affixes) {
      ...
      if (a.type === 'DevotionIncrementNodeAffixDefinition') {
        const bucket = (a.eDevotionCategory as any) as 'Red' | 'Green' | 'Blue' | undefined;
