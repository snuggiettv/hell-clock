...
function mapStatKeyToSection(k) {
    const s = (k || '').toLowerCase();
    if (/(orb|agility|power)/.test(s))
        return 'ORBS';
    if (/(move|movement|attack|cast|evasion|cooldown|speed)/.test(s))
        return 'SPEED';
    if (/(crit|damage|ignite|plague|wither|blight|fire|lightning|magic|physical|poison|bleed|base)/.test(s))
        return 'DAMAGE';
    if (/(life|hp|maxlife|mana|barrier|resist|armor|defense|potion|ward)/.test(s))
        return 'SURVIVAL';
    if (/(gold|reroll|cost)/.test(s))
        return 'ECONOMY';
    return 'MISC';
}
// legacy (pre percent-aware) behavior: always show additive as %
function displayFor(math, acc) {
...
function normalizeContrib(math, raw) {
    if (math === 'Multiplicative') {
...
export function buildStatGroups(rows) {
    var _a;
    const groups = new Map();
    for (const r of rows) {
        const key = String(r.statKey || '').trim();
        const math = String(r.statModifierType || 'Additive');
        if (!key)
            continue;
        if (math === 'Unknown')
            continue;
    ...
    for (const acc of groups.values()) {
        ...
        (bySection[_a = acc.section] || (bySection[_a] = [])).push(line);
    }
