export const MASTER_URL = "https://raw.githubusercontent.com/snuggiettv/hellclock-data-export/refs/heads/main/data/Constellations.json";
...
const slugify = (s) => String(s || "")
    ...
    .replace(/(^-|-$)/g, "");
// ---------- helpers ----------
const get = (o, path) => path.reduce((v, k) => (v != null ? v[k] : undefined), o);
// True only for the constellation objects we care about
function isConstellationEntry(x) {
    if (!x || typeof x !== "object" || Array.isArray(x))
        return false;
    if (x.type === "ConstellationDetails")
        return true;
    const d = x.definition;
    return !!(d && typeof d === "object" && !Array.isArray(d));
}
function englishName(def, group) {
    const arrs = [
        get(def, ["nameKey"]),
    ...
    for (const arr of arrs) {
        if (Array.isArray(arr)) {
            const en = arr.find((k) => k?.langCode === "en")?.langTranslation;
            if (en)
                return String(en).trim();
        }
    }
    const s = get(def, ["name"]) ||
        get(group, ["name"]) ||
        get(def, ["displayName"]) ||
        get(group, ["displayName"]);
    if (s)
        return String(s).replace(/\s*-\s*Constellation\s+Definition$/i, "").trim();
    return String(get(def, ["id"]) ?? get(group, ["id"]) ?? "Unnamed");
}
function fileBaseName(def, group, label) {
    const raw = get(def, ["name"]) || get(group, ["name"]);
    if (raw)
        return String(raw).replace(/\s*-\s*Constellation\s+Definition$/i, "").trim();
    if (label && label !== "Unnamed")
        return label;
    const id = get(def, ["id"]) ?? get(group, ["id"]);
    return id ? String(id) : "Unnamed";
}
const buildDataUrl = (baseName) => `https://raw.githubusercontent.com/snuggiettv/hellclock-data-export/refs/heads/main/data/${encodeURIComponent(baseName)}.json`;
// ---------- main ----------
export async function fetchRegistryFromMaster() {
    const res = await fetch(MASTER_URL);
    if (!res.ok)
        throw new Error(`Failed to fetch constellations: ${res.status}`);
    const master = await res.json();
    // Flatten to an array, then filter to ONLY constellation objects
    const rawList = Array.isArray(master)
        ? master
